...
namespace PridePerception.npcs {
    public class Bezz : NPC {
        ...
        public void LoadFrames() {
            ...
            Sprite[] idleSprites = [Plugin.assets.Get<Sprite>("Images/npcs/Bezz/bezzIdle")];
            animator.AddAnimation("idle", new SpriteAnimation(idleSprites, 1.0f));

            Sprite[] walkSprites = [];
            for (int i = 0; i < 12.0f; i++)
                walkSprites = walkSprites.AddToArray(Plugin.assets.Get<Sprite>("Images/npcs/Bezz/bezzWalk" + i));
            animator.AddAnimation("walk", new SpriteAnimation(walkSprites, 1.0f));

            Sprite[] happyTalkSprites = [];
            for (int i = 0; i < 2.0f; i++)
                happyTalkSprites = happyTalkSprites.AddToArray(Plugin.assets.Get<Sprite>("Images/npcs/Bezz/bezzHappyTalk" + i));
            animator.AddAnimation("happyTalk", new SpriteAnimation(happyTalkSprites, 0.1f));

            Sprite[] upsetTalkSprites = [];
            for (int i = 0; i < 2.0f; i++)
                upsetTalkSprites = upsetTalkSprites.AddToArray(Plugin.assets.Get<Sprite>("Images/npcs/Bezz/bezzUpsetTalk" + i));
            animator.AddAnimation("upsetTalk", new SpriteAnimation(upsetTalkSprites, 0.1f));
    ...
    public class BezzPromptState(Bezz _bezz, NavigationState _navigationState, PlayerManager _pm) : BezzBaseState(_bezz, _navigationState) {
        ...
        public override void Enter() {
            ...
            bezz.audio.FlushQueue(true);
            bezz.audio.QueueAudio(Plugin.assets.Get<SoundObject>("Sounds/npcs/Bezz/bezzPrompt0"));
            bezz.audio.QueueAudio(Plugin.assets.Get<SoundObject>("Sounds/npcs/Bezz/bezzPrompt1"));
            bezz.animator.ChangeSpeed(1.0f);
            ...
            MusicManager musicManager = Singleton<MusicManager>.Instance;
            musicManager.StopMidi();
            musicManager.PlayMidi(Plugin.assets.Get<string>("Midis/npcs/Bezz/FlagCatch" + UnityEngine.Random.Range(0, 2)), false);
    ...
    public class Flag : MonoBehaviour, IClickable<int> {
        ...
        public Flag Initialize(Bezz _bezz, Cell _cell, int _flagType) {
            ...
            cell = _cell;
            flagType = _flagType;
            mapMarker = MapMarkerUtil.Initialize(bezz.ec.map, cell, Plugin.assets.Get<Sprite>("Images/npcs/Bezz/Flag/mapMarker"), Plugin.assets.Get<Sprite>("Images/npcs/Bezz/Flag/mapIcon"));
            ...
            spriteRenderer.sprite = Plugin.assets.Get<Sprite>("Images/npcs/Bezz/Flag/" + flagType + "0");
            spriteRenderer.material = ObjectCreators.SpriteMaterial;

            CustomSpriteRendererAnimator animator = renderer.AddComponent<CustomSpriteRendererAnimator>();
            animator.renderer = spriteRenderer;

            Sprite[] idleSprites = [];
            for (int i = 0; i < 2.0f; i++)
                idleSprites = idleSprites.AddToArray(Plugin.assets.Get<Sprite>("Images/npcs/Bezz/Flag/" + flagType + "" + i));
